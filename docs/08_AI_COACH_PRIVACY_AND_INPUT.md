...
- 可构建 `weekly_report`；Sprint 9A 另提供无 UI 入口的 `daily_insight` 开发合同；
...
Sprint 8D 中，撤销 Consent 后仍允许按已有 request ID 执行状态 GET 和恢复已经发生的结果。该请求不发送新的业务输入、不调用 Provider，也不会创建新 pending；新 Preview 生成与 Generate POST 仍被禁止。
...
Daily Insight 使用调用方明确给出的一个本地自然日，`period.start_date == period.end_date`，身份为 input schema 1、`daily_insight`、`daily-insight-v1`。Daily 只支持 `today_metrics`、`health_metrics`、`journal_reflections`，明确拒绝 `growth_summary` 与 `active_goals`。选中但无记录保存为空数组，未选中则不出现 data key；`null` 与显式 `0` 保持不同。详细合同见 `docs/13_DAILY_INSIGHT_FOUNDATION.md`。
...
Sprint 8D Server Ledger 不保存本合同 Payload、Canonical JSON、Sources、source IDs 或 Journal 原文。它会短期保存已通过输出 Schema 验证的结构化结果和渲染 Markdown，用于恢复丢失响应；这些输出可能包含敏感摘要。最小 request/hash tombstone 会在输出清理后继续保留以防相同 request ID 再次调用。Server Ledger 不是用户 History，本地 `ai_reports` 仍是事实来源。
