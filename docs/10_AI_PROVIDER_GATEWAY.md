...
## Server-only Operations Boundary

Sprint 14A.3 keeps Provider invocation behind `AiGenerationService` and adds a
separate read-only operations layer over the existing generation and usage
ledgers. `python -m app.maintenance.rebirth_ai` can inspect aggregate usage,
safe configuration readiness, operational alerts, and ledger consistency.

The operations layer cannot invoke a Provider, expose a public API, accept a
