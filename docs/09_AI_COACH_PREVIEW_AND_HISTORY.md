...
- 列表路由：`/ai-coach`
- Daily 路由：`/ai-coach/daily/:targetDate`
...
- Weekly 可选 Growth Summary、Today Metrics、Health Metrics、Journal Reflections；Daily 只允许 Today、Health、Journal；
...
Daily 与 Weekly 的 Preview/Generation 使用不可变 `AiInsightRequestContext` 和 Riverpod family 隔离。Daily 上下文包含固定的 `targetDate`；不同日期、不同报告类型不共享 Selection、Bundle、Hash、reusable 或 loading 状态。

最终确认前及确认后提交前都会使用同一 context/selection 重新调用 Input Assembler，并比较 report type、period、prompt version、scopes 和 input hash。任何变化都会阻止 pending、Binding 和 POST，并要求用户重新查看最新 Preview。

## 5. Local History and Detail

History 通过 `AiReportRepository.listRecent()` 读取本地 `ai_reports`，混合显示 Daily 与 Weekly，并按 requestedAt 降序显示类型、周期、
...
Daily 在 History/Detail 中显示单一目标日期，不渲染重复日期范围；Detail 提供 Today/Journal 来源页入口。任何建议都不会自动写回源记录。
