...
AI Coach 当前只有本地 Request Preview 与本地报告 History。当前没有 AI Provider、模型请求、报告生成、
聊天、后台任务或网络发送。Preview 不创建 pending report，也不会保存 Input Snapshot。
...
- pending：说明尚未完成，当前没有 Provider，不提供继续处理；
...
成功保存 Server 渲染的 Markdown、严格 structured output、实际 provider/model 并刷新 History；失败只保存受控 error code。超时不自动重试。崩溃遗留 pending 如实展示，本 Sprint 不自动恢复或重发；Server 无报告存储和跨进程强幂等。
