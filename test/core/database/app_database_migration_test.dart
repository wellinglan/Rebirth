...
void main() {
  test('v2 to v16 preserves goals and adds current tables', () async {
  ...
  test('v3 to v16 preserves goals and creates conflict indexes', () async {
    final fixture = await _createDatabaseFixture();
    addTearDown(fixture.dispose);
    final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
    final bootstrap = await original.bootstrapDao.bootstrap(
  ...
  test(
    'v4 to v16 preserves data and supersedes unhydrated legacy conflicts',
    () async {
      final fixture = await _createDatabaseFixture();
      addTearDown(fixture.dispose);
      final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
  ...
  test(
    'v5 to v16 backfills existing binding without claiming legacy data',
    () async {
      final fixture = await _createDatabaseFixture();
      addTearDown(fixture.dispose);
      final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
...
''');
  ...
  test(
    'v6 to v16 preserves legacy quarantine and business sync metadata',
    () async {
      final fixture = await _createDatabaseFixture();
      addTearDown(fixture.dispose);
      final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
...
''');
  ...
  test('v7 to v16 adds nullable remote identity and prompt schema', () async {
    final fixture = await _createDatabaseFixture();
    addTearDown(fixture.dispose);
    final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
    final bootstrap = await original.bootstrapDao.bootstrap(
  ...
  test(
    'v8 to v16 backfills prompt configuration and Journal snapshots',
    () async {
      final fixture = await _createDatabaseFixture();
      addTearDown(fixture.dispose);
      final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
  ...
  test('v9 to v16 backfills immutable AI report version one', () async {
    final fixture = await _createDatabaseFixture();
    addTearDown(fixture.dispose);
    final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
    final bootstrap = await original.bootstrapDao.bootstrap(
  ...
  test(
    'v10 to v16 preserves conflicts and permits AI report conflicts',
    () async {
      final fixture = await _createDatabaseFixture();
      addTearDown(fixture.dispose);
      final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
  ...
  test(
    'v11 to v16 preserves account data and creates feedback storage',
    () async {
      final fixture = await _createDatabaseFixture();
      addTearDown(fixture.dispose);
      final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
  ...
  test('v12 to v16 preserves legacy scores and normalizes on read', () async {
    final fixture = await _createDatabaseFixture();
    addTearDown(fixture.dispose);
    final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
    final bootstrap = await original.bootstrapDao.bootstrap(
  ...
  test('v13 to v16 adds nullable narratives without rewriting data', () async {
    final fixture = await _createDatabaseFixture();
    addTearDown(fixture.dispose);
    final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
    final bootstrap = await original.bootstrapDao.bootstrap(
  ...
  test('v14 to v16 adds AI chat and reconciliation baseline tables', () async {
    final fixture = await _createDatabaseFixture();
    addTearDown(fixture.dispose);
    final original = AppDatabase.forTesting(NativeDatabase(fixture.file));
    final bootstrap = await original.bootstrapDao.bootstrap(
    ...
    await original.customStatement('DROP TABLE ai_chat_messages');
    await original.customStatement('DROP TABLE ai_chat_threads');
    await original.customStatement('PRAGMA user_version = 14');
    await original.close();

    final migrated = AppDatabase.forTesting(NativeDatabase(fixture.file));
    addTearDown(migrated.close);
    expect(migrated.schemaVersion, 16);
    expect(await migrated.select(migrated.aiChatThreads).get(), isEmpty);
    expect(await migrated.select(migrated.aiChatMessages).get(), isEmpty);
    expect(
      await (migrated.select(migrated.userProfiles)
            ..where((row) => row.id.equals(bootstrap.activeUserId)))
          .getSingleOrNull(),
  ...
  test('schema 16 refuses an automatic downgrade to schema 9', () async {
    final fixture = await _createDatabaseFixture();
    addTearDown(fixture.dispose);
