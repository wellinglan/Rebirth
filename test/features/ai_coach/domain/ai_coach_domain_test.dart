...
void main() {
  test('AI report, status, and generation mode mappings are stable', () {
    expect(
      AiReportType.values.map((value) => value.databaseValue),
      [
        'daily_insight',
        'weekly_report',
        'monthly_reflection',
    ...
    expect(
      AiReportStatus.values.map((value) => value.databaseValue),
      ['pending', 'completed', 'failed'],
    );
    expect(
      AiGenerationMode.values.map((value) => value.databaseValue),
      ['manual', 'automatic'],
    );
  });
  ...
  test('AI data scopes expose stable contract values', () {
    expect(
      AiDataScope.values.map((value) => value.contractValue),
      [
        'growth_summary',
        'today_metrics',
        'health_metrics',
  ...
  test('selection rejects empty scopes and defaults snapshot persistence off', () {
    expect(
  ...
  test('enabled authorization requires a non-negative UTC timestamp', () {
    ...
    expect(
      AiDataAuthorization(enabled: true, consentAt: 123).consentAt,
      123,
    );
  });
