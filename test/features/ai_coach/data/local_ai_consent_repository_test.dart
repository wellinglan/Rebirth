...
void main() {
  ...
  test(
    'bootstrap authorization defaults to disabled with no timestamp',
    () async {
  ...
  test(
    'grant stores UTC consent time and explicitly updates updatedAt',
    () async {
  ...
  test(
    'repeated grant is idempotent and preserves first consent time',
    () async {
  ...
  test('authorization survives repository recreation', () async {
    await repository.grant();
    final recreated = LocalAiConsentRepository(
      database: database,
      dateTimeService: DateTimeService(now: () => currentTime),
    );

    final authorization = await recreated.read();

    expect(authorization.enabled, isTrue);
    expect(authorization.consentAt, currentTime.millisecondsSinceEpoch);
  });

  test('authorization is isolated by the active account', () async {
    ...
    await (database.update(database.userProfiles)
          ..where((row) => row.id.equals(accountBId)))
        .write(const UserProfilesCompanion(isActive: Value(false)));
    await (database.update(database.userProfiles)
          ..where((row) => row.id.equals(accountA.activeUserId)))
  ...
  test(
    'grant and revoke preserve cloud sync, profile, and business rows',
    () async {
